    ...
    for (let agent of enemyAgents) {
        ...
        if (agentControl) {
            ...
            gsap.to(agentControl, {
                ...
                overwrite: true,
                onComplete: () => {
                    // Show weapons for ENEMY agents
                    const enemyWpns = document.querySelectorAll('[id^="enemy_"][id$="_wpn"]');
                    enemyWpns.forEach((weapon) => {
                        weapon.style.opacity = "100%";
                    });

                    // Notify opponent to show their weapons too
                    socket.emit("showWeapons");
                },
            });
        }
    }
...
socket.on("showWeapons", () => {
    // When opponent finishes their animation, show your weapons
    wpn.forEach((weapon) => {
        weapon.style.opacity = "100%";
    });
